  ...
  private void initialize() {
    ...
    setOnMouseClicked(
        event -> {
                    ...
                    result -> {
                      switch (result.getConstructor()) {
                        case TdApi.Messages.CONSTRUCTOR:
                          BTTG.getMessages().put(chatId, (TdApi.Messages) result);
                          TdApi.Message[] messages = ((TdApi.Messages) result).messages;

                          logger.info(
                              "{}/{}", messages.length, BTTG.getMessages().get(chatId).totalCount);
          ...
          // TODO: 10/20/18 Make sure the children get cleared out
